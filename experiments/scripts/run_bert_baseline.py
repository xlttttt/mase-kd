#!/usr/bin/env python3
"""Train BERT student baseline (no KD) from the bert_baseline.toml config.

Usage:
    python experiments/scripts/run_bert_baseline.py
    python experiments/scripts/run_bert_baseline.py --config experiments/configs/bert_baseline.toml
    python experiments/scripts/run_bert_baseline.py --epochs 3 --output-dir outputs/bert_baseline_quick
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="BERT baseline (no KD) training")
    p.add_argument("--config", default="experiments/configs/bert_baseline.toml")
...
def main():
    ...
    config = BertKDConfig(
        ...
        kd=DistillationLossConfig(
            alpha=cfg_dict["kd"]["alpha"],  # 0.0 for baseline
