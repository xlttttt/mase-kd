"""Unit tests for mase_kd.core.losses and mase_kd.distillation.losses."""
...
# ---------------------------------------------------------------------------
# distillation.losses (TinyBERT-style feature KD)
# ---------------------------------------------------------------------------


class TestAttentionDistillationLoss:
    def test_zero_when_identical(self):
        from mase_kd.distillation.losses import attention_distillation_loss

        att = torch.randn(2, 4, 8, 8)  # [batch, heads, seq, seq]
        mapping = {0: 0}
        loss = attention_distillation_loss([att], [att], mapping)
        assert loss < 1e-6

    def test_positive_when_different(self):
        from mase_kd.distillation.losses import attention_distillation_loss

        s_att = [torch.randn(2, 4, 8, 8)]
        t_att = [torch.randn(2, 4, 8, 8)]
        mapping = {0: 0}
        loss = attention_distillation_loss(s_att, t_att, mapping)
        assert loss > 0


class TestHiddenDistillationLoss:
    def test_output_shape_and_gradient(self):
        from mase_kd.distillation.losses import HiddenDistillationLoss

        student_dim, teacher_dim = 256, 768
        loss_fn = HiddenDistillationLoss(student_dim, teacher_dim)

        s_hid = torch.randn(2, 8, student_dim, requires_grad=True)  # [B, seq, dim]
        t_hid = torch.randn(2, 8, teacher_dim)
        mapping = {0: 0}

        loss = loss_fn([s_hid], [t_hid], mapping)
        assert loss.dim() == 0
        loss.backward()
        assert s_hid.grad is not None


class TestPredictionDistillationLoss:
    def test_non_negative(self):
        from mase_kd.distillation.losses import prediction_distillation_loss

        s = torch.randn(4, 10)
        t = torch.randn(4, 10)
        loss = prediction_distillation_loss(s, t, T=3.0)
        assert loss.item() >= 0
    ...
    def test_zero_when_identical(self):
        ...
        logits = torch.randn(4, 10)
        loss = prediction_distillation_loss(logits, logits, T=2.0)
        assert loss.item() < 1e-5
...
class TestGenerateLayerMapping:
    def test_4_to_12_uniform(self):
        from mase_kd.distillation.mapping import generate_layer_mapping

        mapping = generate_layer_mapping(4, 12)
        assert mapping == {0: 2, 1: 5, 2: 8, 3: 11}

    def test_2_to_4_uniform(self):
        from mase_kd.distillation.mapping import generate_layer_mapping

        mapping = generate_layer_mapping(2, 4)
    ...
    def test_invalid_ratio_raises(self):
        ...
        with pytest.raises(ValueError):
            generate_layer_mapping(4, 11)  # 11 not divisible by 4
